...
select  case when js['extra_tables'] > 0 and js['extra_columns'] > 0
        ...
        case when js['extra_tables'] > 0 and js['extra_columns'] > 0
             then 'Check ${meta}.report_extra_tables_in_src for extra tables and ${meta}.report_extra_columns_in_src for extra columns in source database ${src}_${sub}.'
             when js['extra_columns'] > 0 and js['extra_tables'] = 0
             then 'Check ${meta}.report_extra_columns_in_src for extra columns in source database ${src}_${sub}.'
             when js['extra_tables'] > 0 and js['extra_tables'] = 0
             then 'Check ${meta}.report_extra_tables_in_src for extra tables in source database ${src}_${sub}.'
        end as message         
from agg
where issue = 'warning'
and tot > 0;