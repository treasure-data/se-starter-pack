...
--
case
  when nullif(lower(ltrim(rtrim("loyalty_tier"))), 'null') is null then null
  when nullif(lower(ltrim(rtrim("loyalty_tier"))), '') is null then null
  else array_join((transform((split(lower(trim("loyalty_tier")),' ')), x -> concat(upper(substr(x,1,1)),substr(x,2,length(x))))),' ','')
end   AS  "trfmd_loyalty_tier",
--
case
